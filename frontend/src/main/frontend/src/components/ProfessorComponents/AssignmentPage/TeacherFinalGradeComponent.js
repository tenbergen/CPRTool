...
function TeacherFinalGradeComponent() {
    ...
    const {peerReviewFiles, peerReviewTeamFiles} = useSelector((state) => state.peerReviews)
    ...
    const onDownloadClick2 = async (fileName) => {
        return (
            `${process.env.REACT_APP_URL}/manage/courses/${courseId}/assignments/${assignmentId}/download/${fileName}`
        );
    }
        ...
        <div className="ap-component">
            ...
            <div className="ap-assignmentArea">
                <div className="ap-component-links">
                    <h3> Instructions: <br/> <br/>{currentAssignment.instructions}</h3>
                    <h3 id="dueDate" className="dueDate">Due Date: {currentAssignment.due_date}</h3>
                    <div className="ap-teams">
                        <label className="fileLables"> <b> Rubric: <div onClick={() => onFileClick(peerReviewFiles[0])}> {peerReviewFiles[0]} </div> </b> </label><br/>
                        <label className="fileLables"> <b> Template: <div onClick={() => onFileClick(peerReviewFiles[1])}> {peerReviewFiles[1]} </div> </b> </label><br/>
                        <label className="fileLables"> <b> Team Files: {/*peerReviewTeamFiles*/}</b> </label><br/>
                        <label> <b> Peer Review: </b> </label>
